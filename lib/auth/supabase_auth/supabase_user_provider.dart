...
class SuasSupabaseUser extends BaseAuthUser {
  SuasSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> suasSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = SuasSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
